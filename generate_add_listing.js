...
function AddListingPage() {
  ...
  useEffect(() => {
    if (currentStep !== 1) return // Only load map on step 1
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col font-sans overflow-x-hidden">
      {/* Top Header */}
      <header className="flex justify-between items-center px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-50">
         <img src="/logo-new.png" alt="Logo" className="h-8 w-auto object-contain cursor-pointer" onClick={() => router.push('/')} />
         <button onClick={handleSaveAndExit} className="px-5 py-2.5 rounded-full text-sm font-semibold bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-800 transition-colors">
           Save & exit
         </button>
      </header>

      {/* Main Content Area */}
      <main className="flex-1 flex flex-col items-center pb-32 pt-10 px-6">
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 z-50">
         ...
         <div className="h-1.5 w-full bg-slate-100 absolute top-0 left-0">
           <div className="h-full bg-slate-900 transition-all duration-300 ease-in-out" style={{ width: \`\${(currentStep / TOTAL_STEPS) * 100}%\` }} />
         </div>
         
         <div className="flex justify-between items-center px-8 py-4">
            <button onClick={handleBack} className={\`font-semibold text-slate-900 hover:bg-slate-100 px-5 py-2.5 rounded-lg transition-colors \${currentStep === 1 ? 'invisible' : ''}\`}>
              <u className="no-underline">Back</u>
            </button>
            <button 
              onClick={handleNext} 
              disabled={submitting}
              className={\`font-bold text-white px-8 py-3.5 rounded-lg transition-all active:scale-95 flex items-center gap-2 \${currentStep === TOTAL_STEPS ? 'bg-[#E51D53] hover:bg-rose-600' : 'bg-slate-900 hover:bg-slate-800'} disabled:opacity-70 disabled:cursor-not-allowed\`}
            >
              {submitting ? <Loader2 className="w-5 h-5 animate-spin" /> : null}
              {currentStep === TOTAL_STEPS ? 'Publish' : 'Next'}
            </button>
         </div>
      </footer>
    </div>
...
console.log('Successfully wrote new add-listing page!');
